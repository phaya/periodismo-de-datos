...
df_2026 <- df_2026_raw %>% 
  ...
  select(comunidad_autónoma, año, tipo, valor) %>%
  filter(!is.na(valor))
...
year_cols <- setdiff(names(clean), c("tipo", "comunidad_autónoma"))
